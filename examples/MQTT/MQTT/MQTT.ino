...
void setup() {
  ...
  client.setServer(server, 1883);                                 // MQTT Client library initialization
...
void loop() {
  ...
  if (AppVars.SwTimerAck == true){                              // DPA request time flag is set
    ...
    switch (AppVars.State){                                     // select which request is going to send
      case 0:                                                   // coordinator - LED R pulse
        ...
        dpaSuspendDriver();                                     // release SPI bus
        if (client.connected()) {
            client.publish("outTopic", "C LEDR pulse");
        }
        dpaRunDriver();                                         // occupy SPI bus by DPA driver
  ...
  dpaSuspendDriver();                                           // release SPI bus
  if (!client.connected()) {                                    // Run MQTT driver
      mqttReconnect();
  }
  client.loop();
  dpaRunDriver();                                               // occupy SPI bus by DPA driver
